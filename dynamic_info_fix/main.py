...
if __name__ == "__main__":
    ...
    structured_file = '../../data/Drain_result/Spark_full.log_structured.csv'
    template_file = '../../data/Drain_result/Spark_full.log_templates.csv'
    ...
    output_file_grouped = '../../Output/dynamic_extract/grouped/Spark_dynamic_grouped.csv'
    output_file_format = '../../Output/dynamic_extract/format/Spark_dynamic_format.csv'
    pickle_file = '../../Output/dynamic_extract/pickle/Spark.pkl'
    ...
    print(len(broadcast_structured_df))
    broadcast_structured_df = broadcast_structured_df[
        broadcast_structured_df['Content'].str.contains('broadcast', na=False)
    ]
    print(len(broadcast_structured_df))
    broadcast_format_df = dynamic_format_df[dynamic_format_df['Template'] == "<*> <*> in memory on <*> (size: <*> <*> free: <*> <*>"]
    broadcast_structured_df = broadcast_structured_df.iloc[:2000]

    for row in broadcast_structured_df.itertuples(index=False):
        para_list = ast.literal_eval(row.ParameterList)
        template = row.NewTemplate

        index = 0
        while index < len(para_list):
            format_rows = dynamic_format_df[(dynamic_format_df['Template'] == template) & (dynamic_format_df['ParameterIndex'] == index)]
            param = para_list[index]
            index = index + 1